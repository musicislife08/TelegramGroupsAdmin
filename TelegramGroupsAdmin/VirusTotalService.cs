...
public class VirusTotalService(HttpClient client, ILogger<VirusTotalService> logger) : IThreatIntelService
{
    public async Task<bool> IsThreatAsync(string url, CancellationToken ct = default)
    {
        ...
        {
            // Polly rate limiter rejected the request - log as warning (expected behavior)
            logger.LogWarning(ex, "VirusTotal rate limit exceeded for URL: {Url}", url);
            return false; // Fail open - don't block legitimate content due to rate limits
        }
        catch (Exception ex)
        {
            // Unexpected error - log as error
            logger.LogError(ex, "Unexpected error checking VirusTotal for URL: {Url}", url);
