...
public interface IChatAdminsRepository
{
    ...
    /// </summary>
    Task<int> GetPermissionLevelAsync(long chatId, long telegramId);

    /// <summary>
    /// Check if user is an active admin in the specified chat
    /// </summary>
    Task<bool> IsAdminAsync(long chatId, long telegramId);

    /// <summary>
    /// Get all active admins for a specific chat
    /// </summary>
    Task<List<ChatAdmin>> GetChatAdminsAsync(long chatId);

    /// <summary>
    /// Get all chats where a user is an active admin
    /// </summary>
    Task<List<long>> GetAdminChatsAsync(long telegramId);
    ...
    /// </summary>
    Task DeactivateAsync(long chatId, long telegramId);
