...
/// <summary>
/// Tests for login functionality in the WASM UI.
...
/// </summary>
[TestFixture]
public class WasmLoginTests : WasmSharedE2ETestBase
{
    ...
    public async Task Login_WithValidCredentials_RedirectsToHome()
    {
        ...
        var user = await new WasmTestUserBuilder(SharedFactory.Services)
            .WithEmail(TestCredentials.GenerateEmail("wasm-login"))
    ...
    public async Task Login_WithInvalidPassword_ShowsError()
    {
        ...
        var user = await new WasmTestUserBuilder(SharedFactory.Services)
            .WithEmail(TestCredentials.GenerateEmail("wasm-invalid"))
        ...
        // Use Playwright's Expect with auto-retry for async WASM responses
    ...
    public async Task Login_WithUnverifiedEmail_ShowsVerificationError()
    {
        ...
        var password = TestCredentials.GeneratePassword();
        var user = await new WasmTestUserBuilder(SharedFactory.Services)
            .WithEmail(TestCredentials.GenerateEmail("wasm-unverified"))
            .WithPassword(password)
    ...
    public async Task Login_WithLockedAccount_ShowsLockedMessage()
    {
        ...
        var password = TestCredentials.GeneratePassword();
        var user = await new WasmTestUserBuilder(SharedFactory.Services)
            .WithEmail(TestCredentials.GenerateEmail("wasm-locked"))
            .WithPassword(password)
            .WithEmailVerified()
    ...
    public async Task Login_WithDisabledAccount_ShowsDisabledMessage()
    {
        ...
        var password = TestCredentials.GeneratePassword();
        var user = await new WasmTestUserBuilder(SharedFactory.Services)
            .WithEmail(TestCredentials.GenerateEmail("wasm-disabled"))
            .WithPassword(password)
            .WithEmailVerified()
