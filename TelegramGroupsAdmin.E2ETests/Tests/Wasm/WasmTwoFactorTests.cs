...
/// <summary>
/// Tests for two-factor authentication (TOTP) login flow in the WASM UI.
/// Uses WasmSharedE2ETestBase for faster test execution with shared factory.
/// </summary>
[TestFixture]
public class WasmTwoFactorTests : WasmSharedE2ETestBase
{
    ...
    public async Task Login_WithTotpEnabled_RequiresSecondFactor()
    {
        ...
        var user = await new WasmTestUserBuilder(SharedFactory.Services)
            .WithEmail(TestCredentials.GenerateEmail("wasm-totp"))
    ...
    public async Task Login_WithValidTotpCode_RedirectsToHome()
    {
        ...
        var password = TestCredentials.GeneratePassword();
        var user = await new WasmTestUserBuilder(SharedFactory.Services)
            .WithEmail(TestCredentials.GenerateEmail("wasm-totp-valid"))
            .WithPassword(password)
            .WithEmailVerified()
            .WithTotp(enabled: true)
            .AsOwner()
    ...
    public async Task Login_WithInvalidTotpCode_ShowsError()
    {
        ...
        var password = TestCredentials.GeneratePassword();
        var user = await new WasmTestUserBuilder(SharedFactory.Services)
            .WithEmail(TestCredentials.GenerateEmail("wasm-totp-invalid"))
            .WithPassword(password)
            .WithEmailVerified()
            .WithTotp(enabled: true)
            .AsOwner()
    ...
    public async Task Login_WithoutTotp_SkipsVerification()
    {
        ...
        var password = TestCredentials.GeneratePassword();
        var user = await new WasmTestUserBuilder(SharedFactory.Services)
            .WithEmail(TestCredentials.GenerateEmail("wasm-no-totp"))
            .WithPassword(password)
            .WithEmailVerified()
            .AsOwner()
            .BuildAsync();
