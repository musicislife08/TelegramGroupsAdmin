...
/// <summary>
/// Tests for the email verification flow in the WASM UI.
...
/// </summary>
[TestFixture]
public class WasmEmailVerificationTests : WasmSharedE2ETestBase
{
    ...
    public async Task VerifyEmail_WithValidToken_VerifiesAndRedirectsToLogin()
    {
        ...
        var user = await new WasmTestUserBuilder(SharedFactory.Services)
            .WithEmail(TestCredentials.GenerateEmail("wasm-verify"))
    ...
    public async Task VerifyEmail_WithValidToken_CanLoginAfterVerification()
    {
        ...
        var user = await new WasmTestUserBuilder(SharedFactory.Services)
            .WithEmail(TestCredentials.GenerateEmail("wasm-login-after"))
    ...
    public async Task VerifyEmail_WithAlreadyVerifiedUser_RedirectsToLoginWithAlreadyVerified()
    {
        ...
        var user = await new WasmTestUserBuilder(SharedFactory.Services)
            .WithEmail(TestCredentials.GenerateEmail("wasm-already-verified"))
            .WithPassword(TestCredentials.GeneratePassword())
    ...
    public async Task VerifyEmail_WithExpiredToken_ShowsError()
    {
        ...
        var user = await new WasmTestUserBuilder(SharedFactory.Services)
            .WithEmail(TestCredentials.GenerateEmail("wasm-expired-token"))
            .WithPassword(TestCredentials.GeneratePassword())
            .WithEmailVerified(false)
            .WithTotpDisabled()
            .AsOwner()
    ...
    public async Task VerifyEmail_WithUsedToken_ShowsError()
    {
        ...
        var user = await new WasmTestUserBuilder(SharedFactory.Services)
            .WithEmail(TestCredentials.GenerateEmail("wasm-used-token"))
            .WithPassword(TestCredentials.GeneratePassword())
            .WithEmailVerified(false)
            .WithTotpDisabled()
            .AsOwner()
    ...
    public async Task ResendVerification_WithValidUnverifiedEmail_ShowsSuccess()
    {
        ...
        await SharedFactory.EnableEmailVerificationAsync();

        var user = await new WasmTestUserBuilder(SharedFactory.Services)
            .WithEmail(TestCredentials.GenerateEmail("wasm-resend"))
            .WithPassword(TestCredentials.GeneratePassword())
            .WithEmailVerified(false)
            .WithTotpDisabled()
            .AsOwner()
    ...
    public async Task ResendVerification_WithAlreadyVerifiedEmail_ShowsGenericResponse()
    {
        ...
        var user = await new WasmTestUserBuilder(SharedFactory.Services)
            .WithEmail(TestCredentials.GenerateEmail("wasm-already-verified-resend"))
            .WithPassword(TestCredentials.GeneratePassword())
            .WithEmailVerified(true)
            .WithTotpDisabled()
            .AsOwner()
    ...
    public async Task ResendVerification_SendsEmailWithVerificationLink()
    {
        ...
        var user = await new WasmTestUserBuilder(SharedFactory.Services)
            .WithEmail(TestCredentials.GenerateEmail("wasm-check-link"))
            .WithPassword(TestCredentials.GeneratePassword())
            .WithEmailVerified(false)
            .WithTotpDisabled()
            .AsOwner()
    ...
    public async Task UnverifiedUser_CannotLogin()
    {
        ...
        var password = TestCredentials.GeneratePassword();
        var user = await new WasmTestUserBuilder(SharedFactory.Services)
            .WithEmail(TestCredentials.GenerateEmail("wasm-cant-login"))
            .WithPassword(password)
            .WithEmailVerified(false)
            .WithTotpDisabled()
            .AsOwner()
