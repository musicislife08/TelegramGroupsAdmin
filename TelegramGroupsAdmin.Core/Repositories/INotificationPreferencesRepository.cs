...
public interface INotificationPreferencesRepository
{
    ...
    Task<NotificationConfig?> GetByUserIdAsync(string userId, CancellationToken ct = default);

    /// <summary>
    /// Get or create notification preferences for a user
    /// Creates default (empty channels) if not exists
    /// </summary>
    Task<NotificationConfig> GetOrCreateAsync(string userId, CancellationToken ct = default);

    /// <summary>
    /// Save notification preferences (upsert)
    /// </summary>
    Task SaveAsync(string userId, NotificationConfig config, CancellationToken ct = default);
}
