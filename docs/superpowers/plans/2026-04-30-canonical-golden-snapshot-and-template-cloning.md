...
> 4. **Ban-celebration `user_actions` set** — `Telegram/Services/BanCelebrationServiceTests.cs` renders `{bancount}` placeholder. ✅ **RESOLVED in 3A.22 (2026-05-18)** — turned out the test's substrate philosophy (empty slate per test) is incompatible with canonical's reference data, so this test family is the documented exception: empty-template clone + self-contained inline inserts. The 1–7 framing in the original plan note was overstated; only count=3 is ever used. See 3A.22 resolution note below.
>
> **Deliverables:**
...
- [x] **Task 3A.22:** `Telegram/Services/BanCelebrationServiceTests.cs` — `SeedBanActions(int count)` inserts 1–7 ban rows for testing the `{bancount}` placeholder.
  > **RESOLUTION (2026-05-18):** This test family is the **documented exception** to the canonical+reducer+mutator pattern. Every test starts from an empty schema and adds its own scenario via production repositories (`IConfigService.SaveWelcomeAsync`, `IBanCelebrationGifRepository.AddFromFileAsync`, `IBanCelebrationCaptionRepository.AddAsync`). Canonical's reference data (74 captions + 92 GIFs + per-chat configs with ban_celebration enabled across 3 chats) would contaminate the SUT's RNG-driven caption/GIF selection and break the "no captions" / "no GIFs" / "disabled by default" tests.
  >
  > Migration changes:
  >   - Switched from `CreateDatabaseAndApplyMigrationsAsync` (cold migration, ~250–550ms) to `CreateDatabaseFromGoldenTemplateAsync`'s empty-template sibling `CreateDatabaseFromEmptyTemplateAsync` (~50–150ms) for the same fresh-schema substrate, with template-clone speed.
  >   - Made `SeedBanActions` self-contained: inserts both the telegram_users and the user_actions rows it needs. Synthetic IDs in the `9_xxx_xxx_xxx_xxx` canonical identity space (`9100000000000+i`) for collision safety. No dependency on the legacy `00_base_telegram_users.sql` fixture (which is on the cleanup-deletion trajectory).
  >   - Drops the prior plan note about 1–7 enumeration — only one test (`SendBanCelebrationAsync_ReplacesBancountPlaceholder`) exercises `{bancount}`, and only with count=3. The `SeedBanActions(int count)` helper's parameterization is YAGNI but kept for minimum churn.
  >   - **Direct EF inserts in `SeedBanActions` are the documented exception**: `user_actions` has no public production "create ban action" API for tests to route through (the production system writes user_actions internally as a side effect of moderation). `CK_user_actions_exclusive_actor` is satisfied by `system_identifier = "test-bancelebration"` (free-text marker, no FK).
  >
  > 16/16 tests pass.
