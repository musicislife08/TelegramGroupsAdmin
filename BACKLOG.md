...
### OBS-1: Comprehensive Observability with Seq and OpenTelemetry

**Priority:** MEDIUM
**Impact:** Deep production visibility for debugging, performance analysis, and operational insights

**Overview:** Add Seq (datalust/seq:latest) and OpenTelemetry instrumentation for structured logs, distributed traces, and Prometheus metrics. Configuration via optional environment variables (debugging/development use).

#### Phase 1: Docker Infrastructure

**Add Seq Service (`compose/compose.yml`):**
- Service: `seq` (datalust/seq:latest)
- Port: `5341:80` (UI accessible at http://localhost:5341)
- Volume: `./seq-data:/data` (persistent storage)
- Environment: `ACCEPT_EULA=Y`
- Network: `telegram-admin` (existing internal network)

**Update `tga-app` Environment:**
- `SEQ_URL=http://seq:5341` (optional, only if Seq enabled)
- `SEQ_API_KEY=` (optional, ingestion auth)
- `OTEL_SERVICE_NAME=TelegramGroupsAdmin`

#### Phase 2: NuGet Packages

**OpenTelemetry Stack (`TelegramGroupsAdmin.csproj`):**
- `OpenTelemetry.Exporter.OpenTelemetryProtocol` (OTLP exporter for Seq)
- `OpenTelemetry.Extensions.Hosting` (IHostBuilder integration)
- `OpenTelemetry.Instrumentation.AspNetCore` (auto-instrument Blazor Server/API)
- `OpenTelemetry.Instrumentation.Http` (auto-instrument HttpClient: OpenAI, VirusTotal, Telegram)
- `OpenTelemetry.Instrumentation.Runtime` (.NET runtime metrics: GC, thread pool, exceptions)
- `OpenTelemetry.Exporter.Prometheus.AspNetCore` (metrics endpoint at `/metrics`)
- `Npgsql.OpenTelemetry` (PostgreSQL-specific spans, better than generic EF Core)
- `Serilog.Sinks.Seq` (structured log sink with trace correlation)

#### Phase 3: Logging Configuration

**Update `Program.cs` Serilog Setup (lines 11-65):**
- Add conditional Seq sink: `if (!string.IsNullOrEmpty(seqUrl)) WriteTo.Seq(seqUrl, apiKey)`
- Enrich with OpenTelemetry correlation: `.Enrich.WithProperty("TraceId", Activity.Current?.TraceId)`
- Enrich with OpenTelemetry span: `.Enrich.WithProperty("SpanId", Activity.Current?.SpanId)`
- Maintain existing console sink and dynamic log level switching (RuntimeLoggingService)

**Benefits:**
- Searchable logs in Seq UI with filtering by trace ID, span ID, level, namespace
- Automatic correlation between logs and traces (click log → see full request trace)
- Retain existing runtime log level control via Settings UI

#### Phase 4: OpenTelemetry Configuration

**Add to `Program.cs` (after line 90, before `builder.Build()`):**
...
builder.Services.AddOpenTelemetry()
    .WithTracing(tracing => tracing
        ...
        .AddNpgsql()                          // PostgreSQL queries, connection pooling
        .AddSource("TelegramGroupsAdmin.*")  // Custom ActivitySources
        .AddOtlpExporter(options => options.Endpoint = new Uri(seqUrl)))
    .WithMetrics(metrics => metrics
        ...
        .AddMeter("TelegramGroupsAdmin.*")   // Custom meters
        .AddPrometheusExporter());           // Expose /metrics endpoint
```

**Resource Attributes:**
- `service.name=TelegramGroupsAdmin`
- `service.version={Assembly.GetEntryAssembly().GetName().Version}`
- `deployment.environment={ASPNETCORE_ENVIRONMENT ?? "Production"}`

**Map `/metrics` Endpoint:**
- Add `app.MapPrometheusScrapingEndpoint()` in middleware pipeline
- Accessible at http://localhost:8889/metrics (Prometheus scrape target)

#### Phase 5: Custom Instrumentation

**5.1 Create Shared Telemetry Constants (`TelegramGroupsAdmin/Telemetry/TelemetryConstants.cs`):**
...
public static class TelemetryConstants
{
    ...
    // Counters
    public static readonly Counter<long> SpamDetections = Metrics.CreateCounter<long>("spam_detections_total");
    public static readonly Counter<long> FileScanResults = Metrics.CreateCounter<long>("file_scan_results_total");
    public static readonly Counter<long> JobExecutions = Metrics.CreateCounter<long>("job_executions_total");

    // Histograms
    public static readonly Histogram<double> SpamDetectionDuration = Metrics.CreateHistogram<double>("spam_detection_duration_ms");
    public static readonly Histogram<double> FileScanDuration = Metrics.CreateHistogram<double>("file_scan_duration_ms");
}
```

**5.2 High-Priority Instrumentation Targets:**

**Spam Detection Pipeline** (`TelegramGroupsAdmin.SpamDetection/Services/*`):
- Wrap `ContentCheckCoordinator.CheckAsync()` in span from `TelemetryConstants.SpamDetection`
- Add child spans for each algorithm execution (StopWords, CAS, Similarity, OpenAI, etc.)
- Tag spans: `message_id`, `chat_id`, `algorithm_name`, `result` (spam/ham), `confidence`, `processing_time_ms`
- Increment counter: `SpamDetections.Add(1, new("algorithm", algorithmName), new("result", isSpam ? "spam" : "ham"))`
- Record histogram: `SpamDetectionDuration.Record(stopwatch.ElapsedMilliseconds, new("algorithm", algorithmName))`

**File Scanning** (`FileScanJob.cs`, `ClamAVScannerService.cs`, `VirusTotalScannerService.cs`):
- Wrap `FileScanJob.ExecuteAsync()` in span from `TelemetryConstants.BackgroundJobs`
- Add child span for `ClamAVScannerService.ScanAsync()` from `TelemetryConstants.FileScanning`
- Add child span for `VirusTotalScannerService.ScanAsync()` with nested spans for hash lookup, upload, polling
- Tag spans: `file_id`, `file_size`, `scan_tier`, `clamav_result`, `virustotal_result`, `malicious_count`
- Increment counter: `FileScanResults.Add(1, new("tier", tier), new("result", isMalicious ? "malicious" : "clean"))`
- Record histogram: `FileScanDuration.Record(stopwatch.ElapsedMilliseconds, new("tier", tier))`

**Message Processing** (`MessageProcessingService.cs`):
- Wrap processing loop iteration in span from `TelemetryConstants.MessageProcessing`
- Tag spans: `message_id`, `chat_id`, `message_type` (new/edit), `processing_time_ms`
- Gauge for queue depth: `QueueDepth.CreateObservableGauge(() => _messageQueue.Count)`

**TickerQ Job Execution** (create `TelegramGroupsAdmin/Jobs/BaseTickerQJob.cs` or wrapper):
- Wrap all job executions in span from `TelemetryConstants.BackgroundJobs`
- Tag spans: `job_name`, `payload_type`, `success`, `retry_count`, `error_message` (if failed)
- Increment counter: `JobExecutions.Add(1, new("job_name", jobName), new("status", success ? "success" : "failure"))`
- Record histogram: `JobDuration.Record(stopwatch.ElapsedMilliseconds, new("job_name", jobName))`
- **Implementation:** Add base class or extension method that all TickerQ jobs inherit/use

**External HTTP Calls** (auto-instrumented, but add custom tags via `HttpClientFactory` configuration):
- OpenAI: Add tags for `model`, `max_tokens`, `response_tokens`, `finish_reason`
- VirusTotal: Add tags for `endpoint`, `rate_limit_remaining`, `quota_used`
- Telegram Bot API: Add tags for `method_name`, `chat_id`, `user_id`

#### Phase 6: Testing & Validation

**6.1 Build and Deploy:**
- Run `dotnet build` to verify NuGet package compatibility
- Run `docker compose -f compose/compose.yml up -d` to start Seq + app
- Access Seq UI at http://localhost:5341

**6.2 Verify Telemetry Flow:**
- **Logs:** Check Serilog logs appearing in Seq with structured properties
- **Trace Correlation:** Filter logs by `TraceId`, verify full request context
- **Traces:** Navigate Blazor page, verify ASP.NET Core spans in Seq Traces tab
- **Metrics:** Access http://localhost:8889/metrics, verify Prometheus exposition format

**6.3 Smoke Test Key Scenarios:**
1. Upload file via Blazor → verify file scanning spans with ClamAV/VirusTotal children
2. Send spam message via Telegram → verify spam detection spans with algorithm children
3. Trigger background job → verify TickerQ instrumentation with job name tags
4. Make external API call → verify HTTP client traces with custom tags
5. Check `/metrics` → verify custom counters incrementing

**6.4 Performance Validation:**
- Measure overhead of tracing (should be <5ms per request)
- Check memory usage increase (OpenTelemetry batching should be <50MB)
- Verify no log loss during high message volume (100+ messages/min)
...
```markdown
.NET 9.0, Blazor Server, MudBlazor 8.13.0, PostgreSQL 17, EF Core 9.0, TickerQ 2.5.3,
OpenAI API, VirusTotal, SendGrid, Seq (datalust/seq:latest), OpenTelemetry
```

Add to **Configuration** section:
```markdown
**Observability (Optional)** (environment variables):
- `SEQ_URL`: Seq server URL (e.g., http://seq:5341) - if not set, logs only to console
- `SEQ_API_KEY`: Seq API key for ingestion authentication (optional)
...
```markdown
**Observability:**
- Seq not receiving logs: Check SEQ_URL, verify network connectivity (docker network inspect telegram-admin)
- Missing traces: Verify ActivitySource registration in Program.cs, check OTLP exporter configuration
- `/metrics` returning 404: Ensure `app.MapPrometheusScrapingEndpoint()` called after `app.UseRouting()`
- High memory usage: Configure OpenTelemetry batch size limits, adjust sampling rate
- Seq retention: Configure retention policies in Seq UI (default 7 days)
```

**7.2 Add Observability Section to README (if exists):**
- Document Seq UI access (http://localhost:5341)
- Explain optional setup (app works without Seq, just no centralized logs/traces)
- Link to Prometheus metrics endpoint (http://localhost:8889/metrics)
- Example queries for Seq (filter by trace ID, search logs by spam algorithm, etc.)

#### Implementation Strategy

**Incremental Approach:**
1. **Phase 1-2:** Add Seq service + NuGet packages (no code changes, safe to deploy)
2. **Phase 3-4:** Add auto-instrumentation (ASP.NET Core, HttpClient, EF Core) - validate working
3. **Phase 5:** Add custom spans incrementally (one service at a time, measure overhead)
4. **Phase 6-7:** Testing and documentation

**Rollback Strategy:**
- If Seq causes issues: Remove `SEQ_URL` env var, app reverts to console logging
- If tracing overhead too high: Disable custom spans, keep auto-instrumentation only
- If metrics endpoint interferes: Remove `MapPrometheusScrapingEndpoint()`, keeps traces/logs

**Backward Compatibility:**
- All Seq/OpenTelemetry integration optional via environment variables
- App behavior identical if Seq not configured (logs to console, no centralized traces)
- No database migrations required (purely additive observability layer)

**Performance Considerations:**
- OpenTelemetry batching minimizes overhead (<5ms per operation)
- Sampling can be configured if needed (e.g., 10% of traces for high-volume scenarios)
- Seq retention policies prevent unbounded disk usage (configure in UI after deployment)

#### Benefits

**Development:**
- Trace message flow from Telegram → spam detection → database → response
- Debug TickerQ job failures with full context (payload, retry attempts, stack traces)
- Measure algorithm performance (identify slow checks for ML-5)

**Production:**
- Real-time monitoring of spam detection accuracy via metrics
- Track file scanning throughput and virus detection rates
- Correlate logs across distributed operations (message processing → background jobs)
- Historical performance analysis (compare pre/post optimization changes)

**Operational:**
- Single dashboard for all telemetry (logs, traces, metrics in Seq UI)
- Searchable structured logs (filter by chat ID, user ID, algorithm name, etc.)
- Prometheus metrics for external monitoring tools (Grafana, Datadog, etc.)
- Trace correlation for debugging complex flows (e.g., "why was this user banned?")

#### Estimated Effort

**Files Modified:** ~20-25 files
- Program.cs (Serilog + OpenTelemetry setup)
- compose/compose.yml (Seq service)
- CLAUDE.md (documentation)
- 15-20 service files (custom instrumentation)

**New Files:** ~3 files
- TelemetryConstants.cs (shared ActivitySources and Meters)
- BaseTickerQJob.cs or similar (job instrumentation wrapper)
- README updates or observability guide

**NuGet Packages:** +8 packages
**Docker Services:** +1 (Seq)

**Testing Time:** ~2-3 hours (smoke tests, performance validation, documentation verification)
