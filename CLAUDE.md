...
**4.17**: File Scanning - ClamAV (local virus scan, Docker sidecar port 3310) + VirusTotal File API (fallback for suspicious), file types (.exe/.dll/.zip/.rar/.7z/.pdf/.doc/.xls/.apk/.dmg/.pkg), FileScanningCheck always_run=true by default, flow: ClamAV first → if suspicious upload to VirusTotal → verdict. Trusted/admin: delete+notify. Regular: ban+delete
