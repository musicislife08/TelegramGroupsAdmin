...
**4.6** ✅: /tempban command - Full implementation with flexible duration parsing (5m/1h/24h/5min/1hr formats), TickerQ TempbanExpiryJob for auto-unrestriction (replaces Telegram `until_date` for cross-chat bans), cross-chat global temp bans (user_actions with ExpiresAt field), ModerationActionService.TempBanUserAsync(), proper error handling and logging, registered in CommandRouter and dependency injection. DM notifications: Unified SendTempBanNotificationAsync sends tempban details + rejoin links for all managed chats via IUserMessagingService and IChatInviteLinkService. UI integration: TempBanDialog (duration presets + custom), Messages page (per-message action), UserDetailDialog (user-level action). All use shared dialog and service for consistency. Bot response auto-delete: CommandResult.DeleteResponseAfterSeconds dynamically set to match ban duration via MessageProcessingService.ScheduleMessageDeleteAsync. TickerQ timezone fix: All 6 job scheduling locations use DateTimeOffset.UtcNow.UtcDateTime pattern (MessageProcessingService, ModerationActionService, WelcomeService) to prevent timezone bugs.
...
**4.19** ✅: Actor System - Exclusive Arc pattern (3 nullable FKs: web_user_id, telegram_user_id, system_identifier + CHECK constraint). Migration complete: Added actor columns to user_actions, detection_results, user_tags, stop_words, admin_notes (5 tables). Backfilled legacy data (web_admin→web_user_id). Dropped legacy issued_by/added_by string columns. Updated all repositories with LEFT JOIN queries for actor display name resolution. Actor record type with factory methods (FromWebUser, FromTelegramUser, FromSystem). Bot protection enhanced to capture bot identity before banning. Benefits: Real referential integrity, type-safe attribution, efficient queries, proper audit trail. All actions now show actor badges in UI (Auto-Detection, Bot Protection, web user email, Telegram username).
